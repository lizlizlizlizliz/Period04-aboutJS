...
//数组：用一个变量存储一组值，用关键字Array来声明。
var beatles = Array(4);

//填充：向数组中添加元素，需要给出新元素的值和存放位置即元素的下标。
beatles[0] = "John";

//可以在声明数组时同时对它进行填充。
var beatles = Array( "John", "Paul", "George", "Ringo");

//数组元素可以是数值、字符串、布尔值、变量、另一个数组的元素以及其他数组。
...
//关联数组：在填充元素时为每个新元素明确地给出下标，且下标不必局限于使用整数数字，可以用字符串。
var lennon = Array();
lennon["name"] = "John";
lennon["year"] = "1940";
lennon["living"] = false;
...
//包含在其中的文本内容是另一个节点，是<h1>元素的第一个子节点。
...
//尽量少访问DOM。比如可以将要多次使用的对象存储在一个变量中，可以减少搜索DOM的次数。
//合并放置脚本。可以减少加载页面时发送的请求数量。
//压缩脚本。减少空格和注释，精简代码量。
